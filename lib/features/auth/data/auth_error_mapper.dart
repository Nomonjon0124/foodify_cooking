...
enum AuthOperation { login, register, googleSignIn, resendConfirmation }
...
abstract final class AuthErrorMapper {
  static String mapAuthException(
    ...
    if (operation == AuthOperation.resendConfirmation) {
      if (error.code == 'user_already_confirmed' ||
          normalized.contains('already confirmed')) {
        return AuthFailureMessages.resendUserAlreadyConfirmed;
      }
      if (error.statusCode == '429' ||
        ...
        return AuthFailureMessages.resendRateLimited;
      }
    }
    if (error.statusCode == '429' ||
        _hasAny(normalized, const [
  ...
  static String _defaultFor(AuthOperation operation) {
    return switch (operation) {
      ...
      AuthOperation.resendConfirmation => AuthFailureMessages.resendFailed,
    };
  }
